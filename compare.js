...
async function getWeather(url, num) {
  ...
  if (data.cod === "404")
    ...
    document.getElementById(
      `text-found-${num}`,
    ).textContent = `Ville trouvée : ${data.name}`;
  }
}
  ...
  if (cities.every((city) => city)) {
    ...
    const mess =
      diff == 0
        ...
        : diff > 0
        ? `${diff} degrés de plus`
        : `${Math.abs(diff)} degrés de moins`;

    document.getElementById(
      "title",
    ).textContent = `En ce moment il fait ${mess} à ${cities[1].name}`;
    document.getElementById(
      "description",
    ).textContent = `Il fait ${temp0}°C à ${cities[0].name} et ${temp1}°C à ${cities[1].name}.`;
  }
}
